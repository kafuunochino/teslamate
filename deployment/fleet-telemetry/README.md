...
Completed trips share one calculation in list, route detail, home, driving and analysis. Battery-pack energy differences take precedence when historical changes can reconstruct both boundary states. Since Tesla sends changes rather than periodic snapshots, use the latest preceding state, preserving the original timestamp and carrying it for at most ten minutes. If the first state is received after departure, allow at most 30 seconds and require at least 90% of the trip duration to follow that first observation. Require a distinct observation during the trip, reject invalid changes and expired states, and never substitute a post-trip or current reading for the historical end state. Missing historical coverage uses the selected legacy range coefficient. A paired key alone does not make historical estimates official measurements. EnergyRemaining is a nominal battery estimate and can change with BMS recalibration; LifetimeEnergyUsed is not treated as net trip consumption.

DCChargingEnergyIn is battery-side energy for both AC and DC sessions. ACChargingEnergyIn is used only for confirmed AC sessions. Session counters must start near zero within 30 seconds of the session start, cover both boundary states and never reset or become invalid. An unchanged final counter may be carried to the session end for at most ten minutes; the previous session counter is never carried into a new session start. Input losses require both counters to describe the same interval, within one second at each boundary; missing or inconsistent measurements remain unknown. The AC input/battery difference includes conversion and auxiliary loads, and is not battery degradation or a complete wall-meter measurement.
...
Vehicle configuration now checks the authorized vehicle's reported firmware before adding newer fields. NominalFullPackEnergyKwh, BrickSocMinPercent and LifetimeEnergyChargedKwh require firmware 2026.32 or later. With firmware 2026.26.6 or later and telemetry client 1.3.0, include_fields pairs energy/SOC and AC/DC charging counters and includes energy at BMS state transitions. Older vehicles retain the compatible configuration and existing intervals. After a vehicle software upgrade, apply the telemetry configuration again to enable supported fields.

Energy source labels distinguish battery-pack readings, vehicle charging counters, range estimates and power integrals. Aggregates state whether their inputs are measured, estimated or mixed. A reported nominal battery state remains subject to BMS calibration and temperature; it is not a revenue-grade energy meter or an official SOH measurement.
