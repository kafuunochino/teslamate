  ...
  def configure(vin, access, config) do
    with {:ok, c} <- Config.get(),
      ...
      send_request(
        ...
        [{"authorization", "Bearer " <> access}],
        Jason.encode!(%{"vins" => [vin], "config" => config}),
        :proxy
      )
    else
    ...
    case Application.get_env(:teslamate, :tesla_fleet_http) do
      ...
      _ ->
        headers = [
          ...
          {"user-agent", "ChinoCarData/TeslaMate"} | headers
        ]

        headers =
          if body && !List.keymember?(headers, "content-type", 0),
        ...
        with {:ok, response} <-
               ...
               |> Finch.request(name, receive_timeout: 30_000) do
          case response do
            ...
            %Finch.Response{status: 403} ->
              {:error, :permission_denied}

            %Finch.Response{status: 429} ->
              {:error, :rate_limited}

            %Finch.Response{status: s} ->
              {:error, {:http, s}}
          end
        else
