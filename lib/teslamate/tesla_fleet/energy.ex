defmodule TeslaMate.TeslaFleet.Energy do
  ...
  def record(car_id, field, value, date) when field in @history_fields do
    Repo.insert_all("fleet_energy_samples",
  ...
  def boundaries(intervals, fields) do
    intervals =
      for interval <- intervals,
        ...
        %{
          id: interval.id, car_id: interval.car_id,
    ...
    if intervals == [] do
      ...
      Enum.reduce(result.rows, %{}, fn
        [id, field, first_at, first, last_at, last, duration, start_gap, end_gap, monotonic], acc ->
          ...
          if is_number(first) and is_number(last) and span > 0 and
            ...
            reading = %{
              first: first, last: last, first_at: first_at, last_at: last_at,
              coverage: coverage, monotonic: monotonic
            }
  ...
  def drive_energy(drives) do
    ...
    |> Map.new(fn {id, fields} ->
      ...
      {id, %{
        energy_kwh: sample.first - sample.last, source: :fleet_battery,
        start_energy: sample.first, end_energy: sample.last,
        start_sample_at: sample.first_at, end_sample_at: sample.last_at,
  ...
  def charging_energy(sessions) do
    ...
    Map.new(sessions, fn session ->
      ...
      input = if Map.get(session, :fast_charger) == false and input && input.energy > 0, do: input
  ...
  defp capacity_rows(rows), do: Enum.map(rows, fn [period, value] -> %{period: period, value: value} end)
