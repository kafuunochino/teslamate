  ...
  def init(_) do
    children =
      case Config.get() do
        {:ok, c} ->
          proxy =
            if is_binary(c["proxy_ca_file"]) do
              Finch.child_spec(
                name: TeslaMate.TeslaFleet.ProxyHTTP,
                pools: %{
                  default: [
                    size: 1,
                    conn_opts: [
                      transport_opts: [
                        cacertfile: c["proxy_ca_file"],
                        verify: :verify_peer
                      ]
                    ]
                  ]
          ...
          mqtt =
            if c["mqtt_host"] && c["mqtt_password"] do
              Supervisor.child_spec(
                {Tortoise311.Connection,
                 [
                   client_id: "teslamate-fleet-reader",
                   clean_session: false,
                   server:
                     {Tortoise311.Transport.Tcp,
                      host: c["mqtt_host"], port: c["mqtt_port"] || 1883},
                   user_name: c["mqtt_username"],
...
defmodule TeslaMate.TeslaFleet.Maintenance do
  ...
  def handle_info(:refresh, state) do
    case TeslaMate.TeslaFleet.refresh_if_needed() do
      {:error, reason} ->
        Logger.warning("Tesla Fleet: " <> TeslaMate.TeslaFleet.error_message(reason))
