defmodule TeslaMateWeb.TeslaFleetController do
  ...
  def start(conn, _params) do
    ...
    case TeslaFleet.start_authorization(token) do
      {:ok, url, state} ->
        ...
        conn |> put_resp_cookie(@cookie, cookie,
          encrypt: true, secure: conn.scheme == :https or @secure_cookie, http_only: true,
          same_site: "Lax", max_age: 600, path: "/auth/tesla")
          |> put_resp_header("cache-control", "no-store") |> redirect(external: url)
    ...
    with {:ok, %{"state" => expected, "token" => encoded}} <- Jason.decode(cookie || ""),
      ...
      if Accounts.get_user_by_session_token(token) do
        conn |> put_session(:user_session_token, token)
          ...
          |> assign(:current_user, user) |> assign(:current_user_session_token, token)
  ...
  def disconnect(conn, _) do
    case TeslaFleet.disconnect(conn.assigns.current_user) do
      {:ok, :ok} ->
        conn |> put_flash(:info, "本账号的 Tesla 授权已解除，历史车辆数据已保留，请重新登录")
  ...
  def public_key(conn, _) do
    with {:ok, c} <- Config.get(), path when is_binary(path) <- c["public_key_file"],
         {:ok, pem} <- File.read(path), true <- String.starts_with?(pem, "-----BEGIN PUBLIC KEY-----") do
  ...
  defp render_page(conn, result) do
    ...
    render(conn, "index.html", page_title: "Tesla 连接",
      configured: Config.configured?(), connected: not is_nil(TeslaFleet.connection(user)),
      cars: TeslaFleet.connected_cars(user), domain: c["domain"] || URI.parse(c["origin"] || "").host,
