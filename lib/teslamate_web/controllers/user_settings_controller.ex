defmodule TeslaMateWeb.UserSettingsController do
  ...
  def update_profile(conn, %{"user" => params}) do
    case Accounts.update_profile(conn.assigns.current_user, params) do
      {:ok, _} -> conn |> put_flash(:success, "个人资料已更新") |> redirect(to: "/account")
      {:error, %Ecto.Changeset{} = changeset} -> render_settings(conn, profile_changeset: changeset)
  ...
  def update_password(conn, %{"user" => params}) do
    ...
    with :ok <- Security.authorize_password_change(user, session_token(conn), password, params["code"]),
         {:ok, _} <- Accounts.update_password(user, password, params) do
      conn |> put_flash(:success, "密码已更新，所有设备已登出，请重新登录") |> UserAuth.log_out_user()
    else
      {:error, %Ecto.Changeset{} = changeset} -> render_settings(conn, password_changeset: changeset)
      {:error, reason} -> failed(conn, reason)
    end
  end

  def begin_two_factor(conn, %{"security" => params}) do
    case Security.begin_enrollment(conn.assigns.current_user, session_token(conn), params["password"]) do
      {:ok, _} -> conn |> put_flash(:success, "请在验证器中添加密钥，并输入动态码确认启用") |> redirect(to: "/account#security")
      {:error, reason} -> failed(conn, reason)
    end
  end

  def enable_two_factor(conn, %{"security" => params}) do
    result = Security.enable(conn.assigns.current_user, session_token(conn), params["code"], UserAuth.session_metadata(conn))
    security_result(conn, result, "两步验证已启用，其他登录设备已登出")
  end

  def disable_two_factor(conn, %{"security" => params}) do
    result = Security.disable(conn.assigns.current_user, session_token(conn), params["password"], params["code"], UserAuth.session_metadata(conn))
    security_result(conn, result, "两步验证已关闭，旧恢复码已失效，其他登录设备已登出")
  end

  def recovery_codes(conn, %{"security" => params}) do
    result = Security.regenerate_recovery_codes(conn.assigns.current_user, session_token(conn), params["password"], params["code"], UserAuth.session_metadata(conn))
  ...
  def revoke_device(conn, %{"id" => id}) do
    case Accounts.revoke_session(conn.assigns.current_user, id) do
      ...
      _ -> conn |> put_status(:not_found) |> put_view(TeslaMateWeb.ErrorView) |> render("404.html")
    end
  end
