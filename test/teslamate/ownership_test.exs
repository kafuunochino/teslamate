defmodule TeslaMate.OwnershipTest do
  ...
  setup do
    ...
    users = for prefix <- ["first", "second"] do
      Repo.insert!(%User{email: "#{prefix}-#{System.unique_integer([:positive])}@example.com",
        name: prefix, password_hash: "test-only", password_changed_at: now, role: :admin})
    end
    [first, second] = users
    cars = Enum.map(users, fn user ->
  ...
  defp fence(user, name, extra \\ %{}) do
    {:ok, f} = Locations.create_geofence(user, Map.merge(%{
      name: name, latitude: 26.647, longitude: 106.63, radius: 150}, extra))
    f
  end

  defp charge(car) do
    {:ok, charge} = Log.start_charging_process(car, %{latitude: 26.647, longitude: 106.63}, lookup_address: false)
    {:ok, charge} = Log.update_charging_process(charge, %{charge_energy_added: 10, duration_min: 30})
    charge
  end

  test "exclusive ownership rejects overwriting an existing owner", %{first: first, second: second, car1: car} do
  ...
  test "fence IDs, coordinates and fee rules remain owner scoped even for administrators", %{first: first, second: second} do
  ...
  test "overlapping fences never attach to or price another account's charges", %{first: first, second: second, car1: car1, car2: car2} do
    c1 = charge(car1)
    c2 = charge(car2)
    ...
    assert Repo.get!(ChargingProcess, c1.id).geofence_id == g1.id
    assert Locations.find_geofence(%{car_id: car1.id, latitude: 26.647, longitude: 106.63}).id == g1.id
    assert Locations.find_geofence(%{car_id: car2.id, latitude: 26.647, longitude: 106.63}).id == g2.id
  end

  test "new car ownership discards the previous account's private fence labels", %{first: first, second: second, car1: car} do
    c = charge(car)
