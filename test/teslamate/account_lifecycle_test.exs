defmodule TeslaMate.AccountLifecycleTest do
  ...
  defp confirmation(user, extra \\ %{}) do
    Map.merge(%{
      "email" => user.email, "password" => @password, "acknowledge" => "true"
    }, extra)
  end
  ...
  defp factor(user) do
    Repo.insert!(%Authenticator{
      user_id: user.id, secret: NimbleTOTP.secret(), enabled_at: DateTime.utc_now(),
  ...
  test "database constraints also reject primary deletion and privilege escalation", c do
    statements = [
      ...
      {"UPDATE private.users SET is_system_admin = false, role = 'member' WHERE id = $1", c.admin.id}
    ]
  ...
  test "confirmation and password are required and failed verification stays rate limited", c do
    ...
    assert {:error, :invalid_confirmation} =
      Lifecycle.request_deletion(c.user, c.token, confirmation(c.user, %{"acknowledge" => "false"}))
    for _ <- 1..5 do
      assert {:error, :invalid_verification} =
        Lifecycle.request_deletion(c.user, c.token, confirmation(c.user, %{"password" => "wrong"}))
    end
  ...
  test "request stores seven days and revokes every session, challenge and OAuth state", c do
    ...
    assert DateTime.diff(scheduled.deletion_scheduled_at, scheduled.deletion_requested_at) == 604_800
    ...
    assert {:error, :forbidden} = Lifecycle.request_deletion(c.user, c.token, confirmation(c.user))
  ...
  test "2FA must complete before login cancels deletion", c do
    factor(c.user)
    {:ok, _} =
      Lifecycle.request_deletion(c.user, c.token, confirmation(c.user, %{"code" => "request-code"}))
  ...
  test "admin deletion requires current session, correct target and both configured factors", c do
    ...
    assert {:error, :invalid_verification} =
      Lifecycle.delete_account(c.admin, c.admin_token, c.user.id, confirmation(c.user, %{"password" => "wrong"}))
    ...
    assert Accounts.get_user(c.user.id)
    assert {:ok, []} =
      Lifecycle.delete_account(c.admin, c.admin_token, c.user.id, confirmation(c.user, %{"code" => "request-code"}))
    refute Accounts.get_user(c.user.id)
    assert Accounts.get_user(c.other.id)
  end

  test "deleted accounts lose credentials and bindings while historical records remain admin only", c do
    ...
    {:ok, fence} = Locations.create_geofence(c.user, %{name: "Private Home", latitude: 30, longitude: 100, radius: 100})
    {:ok, other_fence} = Locations.create_geofence(c.other, %{name: "Other Home", latitude: 31, longitude: 101, radius: 100})
    {:ok, drive} = Log.start_drive(car)
    {:ok, charge} = Log.start_charging_process(car, %{date: DateTime.utc_now(), latitude: 30, longitude: 100}, lookup_address: false)
    {:ok, charge} = Log.update_charging_process(charge, %{cost: 42.5, charge_energy_added: 10, duration_min: 30})
    Repo.insert!(%Connection{authorized_by_id: c.user.id, access: "deleted-access", refresh: "deleted-refresh", expires_at: DateTime.add(DateTime.utc_now(), 3600)})
    other_connection = Repo.insert!(%Connection{authorized_by_id: c.other.id, access: "other-access", refresh: "other-refresh", expires_at: DateTime.add(DateTime.utc_now(), 3600)})
    factor(c.user)
    oauth(c.token)

    assert {:ok, [car_id]} = Lifecycle.delete_account(c.admin, c.admin_token, c.user.id, confirmation(c.user))
    ...
    assert {:error, :invalid_or_expired_claim} = Accounts.redeem_vehicle_claim(replacement, old_claim)
    assert {:error, :archived_vehicle} = Repo.transaction(fn ->
      locked = Repo.one(from car in Log.Car, where: car.id == ^car_id, lock: "FOR UPDATE")
  ...
  defp oauth(token) do
    Repo.insert_all("fleet_oauth_states", [%{
  ...
  defp eventually(fun, attempts) do
    if fun.(), do: true, else: (Process.sleep(10); eventually(fun, attempts - 1))
  end
end
