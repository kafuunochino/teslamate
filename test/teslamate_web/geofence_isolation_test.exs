defmodule TeslaMateWeb.GeoFenceIsolationTest do
  ...
  setup %{current_user: owner} do
    ...
    {:ok, own} =
      Locations.create_geofence(owner, %{
        name: "Own Place",
        latitude: 26.64,
        longitude: 106.63,
        radius: 100
    ...
    {:ok, foreign} =
      Locations.create_geofence(other, %{
        name: "Secret Place",
        latitude: 36.64,
        longitude: 116.63,
        radius: 100
  ...
  test "member list and direct edit cannot expose another user's place", %{
    conn: conn,
    own: own,
    foreign: foreign
  } do
    ...
    assert {:error, {:redirect, %{to: "/geo-fences"}}} =
             live(conn, "/geo-fences/#{foreign.id}/edit")
  ...
  @tag platform_role: :member
  test "new fence ignores a forged owner and does not initialize from other cars", %{
    conn: conn,
    current_user: user,
    other: other
  } do
