defmodule TeslaMateWeb.MapSettingsTest do
  ...
  test "shows provider-specific fields without applying drafts", %{conn: conn} do
    ...
    view |> form("#map-settings-form", map_settings: %{provider: "amap", amap_key: "", amap_security_code: ""}) |> render_submit()
  ...
  test "applies only on submit and never renders saved secrets", %{conn: conn} do
    ...
    view
    |> form("#map-settings-form", map_settings: %{provider: "amap", amap_key: key, amap_security_code: code})
    |> render_submit()
