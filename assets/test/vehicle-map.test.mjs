...
import { parseMapPoints, toGCJ02, createAMapAdapter, createVehicleMapHook } from "../js/vehicle-map.mjs";

test("rejects missing, malformed and out-of-range positions without placing a car at zero", () => {
  for (const value of ["oops", "null", "{}", '[{"latitude":null,"longitude":30}]',
    '[{"latitude":"","longitude":30}]', '[{"latitude":true,"longitude":30}]',
    '[{"latitude":91,"longitude":30}]', '[{"latitude":30,"longitude":181}]']) {
    assert.deepEqual(parseMapPoints(value), []);
  }
  assert.deepEqual(parseMapPoints('[{"latitude":"0","longitude":"0"},{"latitude":"39.9","longitude":"116.3"}]'),
...
test("matches the existing mainland coordinate conversion and leaves source data unchanged", () => {
  ...
  assert.deepEqual(toGCJ02({ latitude: 48.8566, longitude: 2.3522 }), [2.3522, 48.8566]);
});

function amap() {
  const calls = { markers: [], maps: [] };
  class Map {
    constructor(_canvas, options) { this.options = options; this.events = {}; calls.maps.push(this); }
    on(name, callback) { this.events[name] = callback; }
    add() {}
    setZoomAndCenter(zoom, point) { this.zoom = zoom; this.center = point; }
    getBounds() { return { contains: () => false }; }
    setCenter(point) { this.center = point; }
    setMapStyle(style) { this.style = style; }
    resize() { this.resized = true; }
    destroy() { this.destroyed = true; }
  }
  class Marker {
    constructor(options) { this.position = options.position; calls.markers.push(this); }
...
test("uses native dark styles and moves the current marker without recreating the map", () => {
  ...
  const adapter = createAMapAdapter(sdk, {}, { theme: "dark", ready() {}, failed() {} });
...
function environment() {
  const element = () => ({
    dataset: {}, classList: { add() {} }, setAttribute() {}, append() {}, replaceChildren() {},
  });
  const win = new EventTarget();
  win.setTimeout = () => 1;
  win.clearTimeout = () => {};
  win.location = { reload() {} };
  const doc = { createElement: element, documentElement: { dataset: { theme: "dark" } } };
...
test("reads the newest position after asynchronous map startup and follows theme changes", async () => {
  ...
  const points = [], themes = [];
  let destroyed = false;
  const hook = createVehicleMapHook((_canvas, options) => {
    assert.equal(options.theme, "dark");
    return { render(value) { points.push(value); }, setTheme(value) { themes.push(value); },
...
test("does not create a map after navigating away while the SDK is loading", async () => {
  ...
  const hook = createVehicleMapHook(() => assert.fail("unexpected fallback"),
    { window: win, document: doc, fetchConfig: async () => ({ provider: "amap" }), loadSDK: () => pending });
  hook.el = element();
  hook.el.dataset.points = '[{"latitude":30,"longitude":110}]';
  hook.mounted();
...
test("shows a recoverable error instead of silently falling back to a foreign provider", async () => {
  const { win, doc, element } = environment();
  const hook = createVehicleMapHook(() => assert.fail("unexpected fallback"),
    { window: win, document: doc, fetchConfig: async () => ({ provider: "amap" }),
